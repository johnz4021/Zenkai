...
reference material), `{{CHECK_REQUIREMENTS}}` (the mechanical shape this problem must
satisfy — selected in code from the round spec's check kind), `{{ROUND_SPEC_JSON}}`
(embedded verbatim in the manifest), `{{TARGET_NOTE}}` (optional emphasis from the
candidate's gap history).
