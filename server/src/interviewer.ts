 ...
 * redact to silence.
 */
export function guard(turn: InterviewerTurn, bugFile: string, prompted = true): InterviewerTurn {
  if (!turn.say || !leaksBugLocation(turn.say, bugFile)) return turn;
  return prompted
...
export function claudeInterviewer(templatePath: string, model = 'sonnet'): Interviewer {
  ...
  return async (ctx) => {
    ...
    return guard(parseTurn(raw), ctx.bugFile, ctx.candidateMessage !== null);
  };
}
